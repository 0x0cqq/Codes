...
namespace Poly_div{
  void div(Poly &_A,Poly &_B,Poly &Q,Poly &R){
    static Poly a,b;
    clear(a = _A),clear(b = _B);
    if(a.size() < b.size()){
      Q.resize(0),R = a;
    ...
    clear(b = _B);
    R = NTT::mul(Q,b);
    for(int i = 0;i < int(b.size())-1;i++) R[i] = (_A[i] - R[i] + mod) % mod;
    R.resize(b.size()-1);
  }
}

Poly pow(Poly &x,ll k,Poly &p){
  static Poly t,tmpq,tmpr,ans;
  t = x,ans = {1};
  for(ll i = k;i;i>>=1){
    if(i & 1){
      ans = NTT::mul(ans,t);
      Poly_div::div(ans,p,tmpq,tmpr);ans = tmpr;
    }
    t = NTT::mul(t,t);
    Poly_div::div(t,p,tmpq,tmpr);t = tmpr;
  }
  return ans;
}
